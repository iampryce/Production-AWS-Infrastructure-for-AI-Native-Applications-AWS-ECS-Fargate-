...
The shape of this was already settled going in: an app-level
...
the "redis-queue-saturation" runbook that will get written later, not just generic
resource pressure.
...
The Cloudflare Tunnel module earlier settled on "data source reads
a secret you create by hand" for values with no Terraform provider path.
This module does it differently, on request: the Slack webhook URL and the
...
Jaeger isn't named in the ops-subnet content list the way
...
"Celery queue monitoring" that doesn't need that. The rule is that
...
yet. Building Terraform for either now would mean guessing
at code that doesn't exist yet, the same "don't build ahead of the work
that needs it" discipline this project already applies elsewhere (ECS's
placeholder images, the empty task-role policies added early on). Nothing in
this module blocks on it.

## Two real bugs, both found via genuine live verification, both fixed on the second pass

Every module this project has actually pushed through CI has hit at least
one first-apply surprise (a missing service-linked role, IAM
policies that had to grow to match real API calls, the Cloudflare Tunnel's
cloudflared repo URL, Flagsmith's SG-description
apostrophe and undersized root volume). This module is no exception, and
...
1. **Flower crash-looped.** The Redis module's `random_password` charset
   ...
   **Forward note for the Celery worker service**: ECS's task definitions pass
   ...
   whatever Celery/kombu broker-URL construction gets written later needs the
...
- Not yet verified, and out of scope for this module specifically: whether
  ...
  (that starts once the FastAPI/Celery application code lands) - the collector is confirmed *running*,
...
- Neither Jaeger's UI nor Flower's UI is reachable from outside the VPC
  ...
  configured to proxy into either one. Same gap already noted for
  Flagsmith's admin UI; a real fix here is wiring
  actual tunnel ingress rules, which is Cloudflare Tunnel follow-up work, not
