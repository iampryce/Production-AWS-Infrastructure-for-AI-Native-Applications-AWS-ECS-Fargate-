...
This backend API is narrow on purpose: submit a generation request,
check its status, retrieve a result. Talk to Postgres via the
Alembic-migrated schema and enqueue work to Celery via Redis.
Nothing here calls an AI provider or does any real generation - that's
the worker's job, which doesn't exist yet.
...
`result_url` (null until the worker fills it in). No separate
...
### A new column, in scope for this API specifically

The existing schema (the earlier migrations) had nowhere to put a result.
Added `result_url` (migration 0003) because this API's own stated goal -
"retrieve a result" - has nothing to retrieve without it. This is
different from reaching into the worker's territory: the column exists now
because this API needs somewhere to read from, not because
anything here decides how the future worker populates it.
...
doesn't exist yet. This keeps FastAPI's code real (a genuine
...
the worker's code to exist first, and without FastAPI and the future worker
needing to import from each other.

### Reused a URL-encoding lesson learned earlier, before hitting it again
...
it goes into the URL. This is the exact bug already found once before
...
The original `env.py` read `DATABASE_URL` directly - reasonable when written,
since no app convention existed yet to build a connection string from
pieces. This backend API established that convention (`DB_HOST`/`DB_NAME`/
`DB_USER`/`DB_SECRET`, matching the ECS task definitions),
...
`psycopg2` (already installed earlier), plain `Session` objects,
...
thin" API with two endpoints - a reasonable point to revisit if this
...
**In real AWS**, after `image-build-deploy.yml` built and
...
so the existing pipeline picked up the new Dockerfile on
...
- `docker-compose.yml` (repo root) is the new fast local-verification
  ...
  here.
- The future worker, when it implements `generate_content`, inherits two
  things from this API: the exact task name/args contract already
  proven on the real queue, and the same percent-encoding requirement for
  any Redis or Postgres URL it builds itself.
- Sentry/LangSmith instrumentation still isn't
  wired in - this API only proves the request/response/queue path
